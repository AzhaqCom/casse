...
class TargetSelector {
  // Cache pour éviter les recalculs multiples par tour
  static cache = new Map()
  static currentTurnKey = null

  /**
   * Génère une clé unique basée sur l'état des entités vivantes
   */
  static generateTurnKey(gameState) {
    const enemies = gameState.combatEnemies || []
    const companions = gameState.activeCompanions || []
    const player = gameState.playerCharacter
    
    const enemyStates = enemies.map(e => `${e.name}:${e.currentHP}`).join(',')
    const companionStates = companions.map(c => `${c.name}:${c.currentHP}`).join(',')
    const playerState = player ? `${player.name}:${player.currentHP}` : ''
    
    return `${enemyStates}|${companionStates}|${playerState}`
  }

  /**
   * Efface le cache si l'état du combat a changé
   */
  static checkAndClearCache(gameState) {
    const newTurnKey = this.generateTurnKey(gameState)
    if (this.currentTurnKey !== newTurnKey) {
      this.cache.clear()
      this.currentTurnKey = newTurnKey
    }
  }

  /**
   * Obtient depuis le cache ou calcule les cibles
   */
  static getCachedTargets(entity, gameState, calculationFn, cacheKey) {
    this.checkAndClearCache(gameState)
    
    const entityKey = entity.name || entity.id || entity.type
    const fullKey = `${entityKey}:${cacheKey}`
    
    if (this.cache.has(fullKey)) {
      return this.cache.get(fullKey)
    }
    
    const result = calculationFn(entity, gameState)
  ...
  static findTargets(entity, gameState) {
    return this.getCachedTargets(entity, gameState, (entity, gameState) => {
      if (entity.type === 'companion') {
        ...
        if (gameState.activeCompanions) {
          ...
          console.log(`🔍 CACHE MISS: TargetSelector.findTargets() calculé pour ${entity.name || entity.type}, ${gameState.activeCompanions.length} compagnons trouvés`)
        }
